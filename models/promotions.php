...
if(isset($_GET['acc']) && ($_GET['acc']=='l')){
	...
	$mySqlFilters= "SELECT idCategory, name, urlPictoNavVals, urlPicto1, pictoHome FROM categories";
	...
	while ($row=mySqli_fetch_array($resultFilters))
	{
		...
		$dades .= '{"idCategory":"'.$row['idCategory'].'", "name":"'.$row['name'].'", "pictoHome":"'.$row['pictoHome'].'", "urlPicto1":"'.$row['urlPicto1'].'" ,"urlPictoNavVals":"'.$row['urlPictoNavVals'].'"}';
		$i++;
	}
	$dades .= "],";
