...
if(isset($_GET['acc']) && ($_GET['acc']=='s')){
	$mySql = "SELECT p.idPromotion, p.image, date_format(p.dataExpireVals,'%d/%m/%y') as dataExpireVals, p.oferVals, p.conditionsVals, s.name FROM promotions p, shops s WHERE s.idShop = p.idShop AND p.idPromotion=".$_GET['idPromo']." ORDER BY p.dataExpireVals";
	$connexio = connect();
	$resultPromotion = mysqli_query($connexio, $mySql);
	disconnect($connexio);

	$dadesPromo = mySqli_fetch_row($resultPromotion);
	$dades='[{"image":"'.$dadesPromo[1].'", "idPromo":"'.$dadesPromo[0].'", "offer":"'.$dadesPromo[3].'", "dataExpire":"'.$dadesPromo[2].'", "conditions":"'.$dadesPromo[4].'", "nameShop":"'.$dadesPromo[5].'"}]';
	echo $dades;
}
?>